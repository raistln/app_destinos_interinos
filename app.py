...
def render_buy_me_coffee():
    """Renderiza el botón de Buy Me a Coffee."""
    st.markdown("""
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        if not mistral_api_key:
            ...
            st.info("💡 Puedes obtener tu API key gratuita en [Mistral AI](https://console.mistral.ai/)")
            st.stop()
...
def main():
    ...
    with st.expander("ℹ️ Sobre esta aplicación"):
        st.markdown("""
        Esta aplicación ayuda a los **interinos de educación en Andalucía** a seleccionar sus destinos 
        preferidos basándose en la proximidad a ciudades de referencia.
        ...
        **Características:**
        ...
        - 🗺️ Cálculo de distancias por carretera
    ...
    if not mistral_api_key and not load_or_create_settings():
        st.warning("⚠️ Por favor, introduce tu API key de Mistral o configúrala en los secrets de Streamlit.")
    ...
    render_buy_me_coffee()
    
    st.markdown("---")
    st.markdown("""
        <div style="text-align: center; color: var(--text-color);">
            <p>Desarrollado para interinos de educación en Andalucía</p>
            <p style="font-size: 0.8rem;">© 2024 - <a href="https://www.linkedin.com/in/samuel-mart%C3%ADn-fonseca-74014b17/" target="_blank">Samuel Martín Fonseca</a></p>
        </div>
    """, unsafe_allow_html=True)
